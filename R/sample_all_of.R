# mzTab-M reference implementation and validation API.
#
# This is the mzTab-M reference implementation and validation API service.
#
# The version of the OpenAPI document: 2.0.0
# Contact: nils.hoffmann@isas.de
# Generated by: https://openapi-generator.tech

#' @docType class
#' @title SampleAllOf
#' @description SampleAllOf Class
#' @format An \code{R6Class} generator object
#' @field name  character [optional]
#'
#' @field custom  list( \link{Parameter} ) [optional]
#'
#' @field species  list( \link{Parameter} ) [optional]
#'
#' @field tissue  list( \link{Parameter} ) [optional]
#'
#' @field cell_type  list( \link{Parameter} ) [optional]
#'
#' @field disease  list( \link{Parameter} ) [optional]
#'
#' @field description  character [optional]
#'
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
SampleAllOf <- R6::R6Class(
  'SampleAllOf',
  public = list(
    `name` = NULL,
    `custom` = NULL,
    `species` = NULL,
    `tissue` = NULL,
    `cell_type` = NULL,
    `disease` = NULL,
    `description` = NULL,
    initialize = function(`name`=NULL, `custom`=NULL, `species`=NULL, `tissue`=NULL, `cell_type`=NULL, `disease`=NULL, `description`=NULL, ...){
      local.optional.var <- list(...)
      if (!is.null(`name`)) {
        stopifnot(is.character(`name`), length(`name`) == 1)
        self$`name` <- `name`
      }
      if (!is.null(`custom`)) {
        stopifnot(is.vector(`custom`), length(`custom`) != 0)
        sapply(`custom`, function(x) stopifnot(R6::is.R6(x)))
        self$`custom` <- `custom`
      }
      if (!is.null(`species`)) {
        stopifnot(is.vector(`species`), length(`species`) != 0)
        sapply(`species`, function(x) stopifnot(R6::is.R6(x)))
        self$`species` <- `species`
      }
      if (!is.null(`tissue`)) {
        stopifnot(is.vector(`tissue`), length(`tissue`) != 0)
        sapply(`tissue`, function(x) stopifnot(R6::is.R6(x)))
        self$`tissue` <- `tissue`
      }
      if (!is.null(`cell_type`)) {
        stopifnot(is.vector(`cell_type`), length(`cell_type`) != 0)
        sapply(`cell_type`, function(x) stopifnot(R6::is.R6(x)))
        self$`cell_type` <- `cell_type`
      }
      if (!is.null(`disease`)) {
        stopifnot(is.vector(`disease`), length(`disease`) != 0)
        sapply(`disease`, function(x) stopifnot(R6::is.R6(x)))
        self$`disease` <- `disease`
      }
      if (!is.null(`description`)) {
        stopifnot(is.character(`description`), length(`description`) == 1)
        self$`description` <- `description`
      }
    },
    toJSON = function() {
      SampleAllOfObject <- list()
      if (!is.null(self$`name`)) {
        SampleAllOfObject[['name']] <-
          self$`name`
      }
      if (!is.null(self$`custom`)) {
        SampleAllOfObject[['custom']] <-
          lapply(self$`custom`, function(x) x$toJSON())
      }
      if (!is.null(self$`species`)) {
        SampleAllOfObject[['species']] <-
          lapply(self$`species`, function(x) x$toJSON())
      }
      if (!is.null(self$`tissue`)) {
        SampleAllOfObject[['tissue']] <-
          lapply(self$`tissue`, function(x) x$toJSON())
      }
      if (!is.null(self$`cell_type`)) {
        SampleAllOfObject[['cell_type']] <-
          lapply(self$`cell_type`, function(x) x$toJSON())
      }
      if (!is.null(self$`disease`)) {
        SampleAllOfObject[['disease']] <-
          lapply(self$`disease`, function(x) x$toJSON())
      }
      if (!is.null(self$`description`)) {
        SampleAllOfObject[['description']] <-
          self$`description`
      }

      SampleAllOfObject
    },
    fromJSON = function(SampleAllOfJson) {
      SampleAllOfObject <- jsonlite::fromJSON(SampleAllOfJson)
      if (!is.null(SampleAllOfObject$`name`)) {
        self$`name` <- SampleAllOfObject$`name`
      }
      if (!is.null(SampleAllOfObject$`custom`)) {
        self$`custom` <- ApiClient$new()$deserializeObj(SampleAllOfObject$`custom`, "array[Parameter]", loadNamespace("rmzTabM"))
      }
      if (!is.null(SampleAllOfObject$`species`)) {
        self$`species` <- ApiClient$new()$deserializeObj(SampleAllOfObject$`species`, "array[Parameter]", loadNamespace("rmzTabM"))
      }
      if (!is.null(SampleAllOfObject$`tissue`)) {
        self$`tissue` <- ApiClient$new()$deserializeObj(SampleAllOfObject$`tissue`, "array[Parameter]", loadNamespace("rmzTabM"))
      }
      if (!is.null(SampleAllOfObject$`cell_type`)) {
        self$`cell_type` <- ApiClient$new()$deserializeObj(SampleAllOfObject$`cell_type`, "array[Parameter]", loadNamespace("rmzTabM"))
      }
      if (!is.null(SampleAllOfObject$`disease`)) {
        self$`disease` <- ApiClient$new()$deserializeObj(SampleAllOfObject$`disease`, "array[Parameter]", loadNamespace("rmzTabM"))
      }
      if (!is.null(SampleAllOfObject$`description`)) {
        self$`description` <- SampleAllOfObject$`description`
      }
    },
    toJSONString = function() {
      jsoncontent <- c(
        if (!is.null(self$`name`)) {
        sprintf(
        '"name":
          "%s"
                ',
        self$`name`
        )},
        if (!is.null(self$`custom`)) {
        sprintf(
        '"custom":
        [%s]
',
        paste(sapply(self$`custom`, function(x) jsonlite::toJSON(x$toJSON(), auto_unbox=TRUE, digits = NA)), collapse=",")
        )},
        if (!is.null(self$`species`)) {
        sprintf(
        '"species":
        [%s]
',
        paste(sapply(self$`species`, function(x) jsonlite::toJSON(x$toJSON(), auto_unbox=TRUE, digits = NA)), collapse=",")
        )},
        if (!is.null(self$`tissue`)) {
        sprintf(
        '"tissue":
        [%s]
',
        paste(sapply(self$`tissue`, function(x) jsonlite::toJSON(x$toJSON(), auto_unbox=TRUE, digits = NA)), collapse=",")
        )},
        if (!is.null(self$`cell_type`)) {
        sprintf(
        '"cell_type":
        [%s]
',
        paste(sapply(self$`cell_type`, function(x) jsonlite::toJSON(x$toJSON(), auto_unbox=TRUE, digits = NA)), collapse=",")
        )},
        if (!is.null(self$`disease`)) {
        sprintf(
        '"disease":
        [%s]
',
        paste(sapply(self$`disease`, function(x) jsonlite::toJSON(x$toJSON(), auto_unbox=TRUE, digits = NA)), collapse=",")
        )},
        if (!is.null(self$`description`)) {
        sprintf(
        '"description":
          "%s"
                ',
        self$`description`
        )}
      )
      jsoncontent <- paste(jsoncontent, collapse = ",")
      paste('{', jsoncontent, '}', sep = "")
    },
    fromJSONString = function(SampleAllOfJson) {
      SampleAllOfObject <- jsonlite::fromJSON(SampleAllOfJson)
      self$`name` <- SampleAllOfObject$`name`
      self$`custom` <- ApiClient$new()$deserializeObj(SampleAllOfObject$`custom`, "array[Parameter]", loadNamespace("rmzTabM"))
      self$`species` <- ApiClient$new()$deserializeObj(SampleAllOfObject$`species`, "array[Parameter]", loadNamespace("rmzTabM"))
      self$`tissue` <- ApiClient$new()$deserializeObj(SampleAllOfObject$`tissue`, "array[Parameter]", loadNamespace("rmzTabM"))
      self$`cell_type` <- ApiClient$new()$deserializeObj(SampleAllOfObject$`cell_type`, "array[Parameter]", loadNamespace("rmzTabM"))
      self$`disease` <- ApiClient$new()$deserializeObj(SampleAllOfObject$`disease`, "array[Parameter]", loadNamespace("rmzTabM"))
      self$`description` <- SampleAllOfObject$`description`
      self
    }
  )
)
